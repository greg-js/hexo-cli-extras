...
module.exports = function modExports(args) {
  ...
  this.load().then(function loadDB() {
    ...
    getArticles(query, locals).then(function loadArticles(articles) {
    ...
    function getArticles(dataType, localData) {
      return Promise.resolve(localData.get(dataType).sort('-date').toArray());
    }
