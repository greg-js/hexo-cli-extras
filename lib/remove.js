...
module.exports = function modExports(args) {
  ...
  this.load().then(function loadDb() {
    ...
    getArticles(locals).then(function loadArticles(articles) {
      var getSelected = new Promise(function makePromise(resolve, reject) {

        var filtered = filterOnName(articles, searchTerms);
        var entries;
      ...
      getSelected.then(function processSelected(selected) {
      ...
      fsStat(src).then(function getStats(stats) {
        if (stats.isFile()) {
          ...
          fsUnlink(src).then(function unlinkFile() {
            ...
            }).catch(function failAssetDir(adErr) {
              console.log(chalk.red('Problem deleting asset dir: ', adErr));
              process.exit();
            });
          });
        }
