...
module.exports = function modExports() {
  ...
  this.load().then(function loadDB() {
    ...
    Promise.resolve(isExileDir(exileDir)).then(function checkExileDir(exists) {
      if (!exists) {
        process.exit();
      }
    }).then(function lsDir() {
      return Promise.resolve(getExiledPosts(exileDir)).then(function checkExiledPosts(files) {
        if (!files) {
